...
| v0.8.0 release | 2026-06-12 | Type-safe WHERE helpers (`sqlh.Eq`, `sqlh.Ne`, `sqlh.Gt`, `sqlh.Gte`, `sqlh.Lt`, `sqlh.Lte`, `sqlh.Like`, `sqlh.In`, `sqlh.IsNull`, `sqlh.IsNotNull`), code comparison examples, performance benchmarks, animated GIF demo, pkg.go.dev badge, backward-compat fix for query.Update/Delete. |
